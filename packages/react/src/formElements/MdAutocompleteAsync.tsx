...
const MdAutocompleteAsync = ({
    ...
    const handleOptionClick = (option: MdAutocompleteAsyncOptionProps) => {
        ...
        setDisplayValue(isSelectedOption(option) ? '' : displayValueAndText ? option.value + ' ' + option.text : option.text);
    ...
    useEffect(() => {
        console.log("autocompleteValue changed");
        if (autocompleteValue) {
            optionsLoader(autocompleteValue)
                .then(result => {
                    setResults(result);
                    if (result && focused) {
                        setOpen(true);
                ...
                {results && results.length > 0 &&
                    ...
                    >
                        {results.map(option => (
                            ...
                            >
                                <div className="md-autocomplete__dropdown-item-text">{displayValueAndText ? `${option.value} ${option.text}` : option.text}</div>
...
export function RequiredAsterisk(): ReactElement {
    return (
        <span className="md-autocomplete__asteriks" aria-label='obligatorisk'> &#42;</span>
    );
}
